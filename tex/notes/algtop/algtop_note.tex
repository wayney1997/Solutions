...
\begin{definition}
	...
	\begin{enumerate}
		\item $x$ is a \textbf{limit point} of $Y$ if $B(x;r) \cap Y$ is infinite for $\forall r>0 \iff \left(B(x;r)\setminus \{x\}\right)\cap Y \neq \phi$ for $\forall r>0$.
		\item $x$ is an \textbf{isolated point} of $Y$ if $\exists r>0$ such that $B(x;r)\cap Y = \{x\}$ $\iff \left(B(x;r)\setminus \{x\}\right)\cap Y = \phi$ for $\exists r>0$.\\
	\end{enumerate}
\end{definition}
